...
class Particle:
  def __init__(self, px, py, pz, ID, energy, phi, pEnergy, status):
    ...
    self.status = status
    self.theta = np.arctan2(float(self.pT),float(pz))
    self.eta2=(-1)*np.log(self.theta/2)
...
### now to graphing #####
  ...
  for item in eventarray: #write output files here?
    ...
    if item.numP!=0 :
      ...
      if part > 600: #this is here for multiplicity cuts, most ridges appear above N=800, setting teh cut to ~ 750 should get all of them
        ct+=1
        print part
...
#unfortunately this leads to empty bins in the method used here and therefore eliminates most of our signal (i.e. big blank graph at the end)
    '''
  print 'Number of events with N > 600 is:',ct,'out of 1000'
...
#### if the file is not input ####
  ...
  print "Usage: python CRMCevent-separator-count.py [PATH/FILE]"
