...
### TODO: need to convert CRMC IDs to CORSIKA IDs, getting errors
        ...
        etaphidiff = root.TH2F('etaphidiff_'+str(i)+'_QGP='+str(QGP)+'_N='+str(part), '#Delta#eta#Delta#phi', 75, -10, 10, 75, -np.pi, np.pi)#used to be 100x100 bins, now 50x50
        for item in f:
...
#unfortunately this leads to empty bins in the method used here and therefore eliminates most of our signal (i.e. big blank graph at the end)
  ...
  c.Print(filename.strip('.hepmc')+'_ROOTetaphi-pTcut'+pptt+'.pdf[') #start pdf
  for item in hists: #graph all events
    ...
    c.Print(filename.strip('.hepmc')+'_ROOTetaphi-pTcut'+pptt+'.pdf') #fill multipage pdf
    i+=1

  #c.Print(filename.strip('.hepmc')+'_ROOTetaphimix.pdf]') #close pdf
  c.Print(filename.strip('.hepmc')+'_ROOTetaphi-pTcut'+pptt+'.pdf]') #close pdf
...
#### if the file is not input ####
  ...
  print "Usage: python CRMCevent-separator-pTcut.py [PATH/FILE] pT (energy < pT)"
