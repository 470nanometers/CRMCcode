...
### now to graphing #####
    ...
    if item.numP!=0 :
      ...
      if part > 900: #this is here for multiplicity cuts, most ridges appear above N=800, setting teh cut to ~ 750 should get all of them
        ...
        for item2 in item.particlelist: #particle lists for each event
          ...
          if item2.ID >95 and item2.eta < 13 and item2.eta > 7: #not photons, EM, or QGP/strings.  Note: muons are ID +- 13, but we'd see late muons from decays not in early shower
  ...
  c.Print(filename.strip('.hepmc')+'_ROOTphiabs-etacut13-7.pdf[') #start pdf
  for item in hists: #graph all events
    ...
    #root.gPad.SetLogz() 
    graphme.SetMinimum(0)#testing y axis min
    graphme.SetMaximum(5)#testing y axis max
    graphme.Draw()
    graphme.Fit("fit")
    #graphme.Draw("contz")
    ...
    c.Print(filename.strip('.hepmc')+'_ROOTphiabs-etacut13-7.pdf') #fill multipage pdf
    i+=1

  c.Print(filename.strip('.hepmc')+'_ROOTphiabs-etacut13-7.pdf]') #close pdf
...
#### if the file is not input ####
  ...
  print "Usage: python CRMCphiabs.py [PATH/FILE]"
