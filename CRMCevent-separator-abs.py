...
### TODO: need to convert CRMC IDs to CORSIKA IDs, getting errors
        ...
        etaphidiff = root.TH2F('etaphidiff_'+str(i)+'_QGP='+str(QGP)+'_N='+str(part), '#Delta#eta#Delta#phi', 75, -6, 6, 75, 0, np.pi)#used to be 100x100 bins, now 50x50
        for item in f:
          deleta=abs(item[0][0]-item[1][0])
          delphi=abs(item[0][1]-item[1][1])
          if delphi > np.pi:
            delphi = 2*np.pi - delphi
          etaphidiff.Fill(deleta, delphi) #should be eta1-eta2 and phi1-phi2 ([e1,p1][e2,p2]); hopefully this is the correct phi
          etaphidiff.Fill((deleta*-1.), delphi) #should be eta1-eta2 and negative phi1-phi2 ([e1,p1][e2,p2]); mirroring across delphi==0
  ...
  totalmixed = root.TH2F('totalmixed', '#Delta#eta#Delta#phi', 75, -6, 6, 75, 0, np.pi)#used to be 100x100 bins, now 50x50
  f=0
  while f < len(histlist): #get first list of etas
    ...
    while j < len(histlist): #get second list of etas; repeat for all etas, no repeats i.e. 1-2, 1-3, 1-4, but no 1-1 or 4-1
      ...
      for item in s:
        ...
        totalmixed.Fill(tdeleta, tdelphi) # get deltas and fill histogram, only one hist needed
        totalmixed.Fill((tdeleta*-1.), tdelphi) # get deltas and fill histogram, only one hist needed, Mirroring Across tdelphi==0
...
#unfortunately this leads to empty bins in the method used here and therefore eliminates most of our signal (i.e. big blank graph at the end)
  ...
  c.Print(filename.strip('.hepmc')+'_ROOTetaphiabs.pdf[') #start pdf
  for item in hists: #graph all events
    ...
    c.Print(filename.strip('.hepmc')+'_ROOTetaphiabs.pdf') #fill multipage pdf
    i+=1
  ...
  totalmixed.GetXaxis().SetTitle("#Delta#eta")
  totalmixed.GetYaxis().SetTitle("#Delta#phi (radians)")
  totalmixed.SetLabelSize(0.025);
  totalmixed.SetLabelSize(0.025,"Y");
  c.Print(filename.strip('.hepmc')+'_ROOTetaphiabs.pdf') #fill multipage pdf
  #c.Print(filename.strip('.hepmc')+'_ROOTetaphimix.pdf]') #close pdf
  c.Print(filename.strip('.hepmc')+'_ROOTetaphiabs.pdf]') #close pdf
...
#### if the file is not input ####crmc_eposlhc_887903004_p_C_1e+08.hepmc
  ...
  print "Usage: python CRMCevent-separator-abs.py [PATH/FILE]"
