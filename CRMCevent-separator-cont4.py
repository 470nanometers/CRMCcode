...
### now to graphing #####
  ...
  for item in eventarray: #write output files here?
    ...
    if item.numP!=0 :
      ...
      if part > 700: #this is here for multiplicity cuts, most ridges appear above N=800, setting teh cut to ~ 750 should get all of them
...
### TODO: need to convert CRMC IDs to CORSIKA IDs, getting errors
        ...
        etaphidiff = root.TH2F('etaphidiff_'+str(i)+'_QGP='+str(QGP)+'_N='+str(part), '#Delta#eta#Delta#phi', 75, -6, 6, 75, -4, 4)#used to be 100x100 bins, now 50x50
        for item in f:
  ...
  totalmixed = root.TH2F('totalmixed', '#Delta#eta#Delta#phi', 75, -6, 6, 75, -4, 4)#used to be 100x100 bins, now 50x50
  f=0
...
#unfortunately this leads to empty bins in the method used here and therefore eliminates most of our signal (i.e. big blank graph at the end)
  ...
  c.Print(filename.strip('.hepmc')+'_ROOTetaphi2-cont.pdf[') #start pdf
  for item in hists: #graph all events
    ...
    c.Print(filename.strip('.hepmc')+'_ROOTetaphi2-cont.pdf') #fill multipage pdf
    i+=1

  #c.Print(filename.strip('.hepmc')+'_ROOTetaphimix.pdf]') #close pdf
  c.Print(filename.strip('.hepmc')+'_ROOTetaphi2-cont.pdf]') #close pdf
