...
class Event:
  def __init__(self, CS, CSe,Im,iCS,numP,numQGP,numpi,numK, numQGPnu):
  ...
  for line in HEPfile:
    ...
    if iterator >4: #dumb workaround to get past the empty line and the first E line so 1st event is not zeros
      ...
      if parts[0]=='E': #start of new event -> reset
        eventarray.append(Event(CS,CSe,Im,iCS,numP,numQGP,numpi,numK,numQGPnu)) 
        Im= 0 #reset
        iCS= 0
        CS= 0
        CSe= 0
...
### now to graphing #####
  ...
  pdf =PdfPages(filename.strip('.hepmc')+'_Kpi.pdf') #put ALL graphs into one PDF
  ...
  for item in QGPmomentum:
    ...
    #print item.ID
    if item.ID == 321 or  item.ID ==311 or  item.ID ==-321: ##K
      zk.append(float(item.pz))
      pTk.append(float(item.pT))
      if str(item.eta) == 'inf' :
        etak.append(50) #if inf, is a divide by zero, meaing pz >>1, meaning very far forward)
      elif str(item.eta) =='-inf':
        etak.append(-15)
      else:
        etak.append(item.eta)
      xFk.append(item.xF)
    if item.ID == 211 or  item.ID ==111 or  item.ID ==-211: ##pi
      z.append(float(item.pz))
    ...
    '''
    if item.ID == 443 or item.ID ==-443: ##J/psi, unfortunately EPOS doesn't DO charmed particles
      zJ.append(float(item.pz))
      pTJ.append(float(item.pT))
      if str(item.eta) == 'inf' :
        etaJ.append(50) #if inf, is a divide by zero, meaing pz >>1, meaning very far forward)
      elif str(item.eta) =='-inf':
        etaJ.append(-15)
      else:
        etaJ.append(item.eta)
      xFJ.append(item.xF)
    '''
  ...
  fig = figure()
  ax = fig.add_subplot(111)
  plt.hist(etak, bins=100, align='left')
  xlabel(xlab)
  ylabel(ylab)
  title(tle)
  plt.savefig(pdf,format='pdf')
  ...
  fig = figure()
  ax = fig.add_subplot(111)
  plt.hist(pTk, bins=100, align='left', log=True)
  #ylim(1,1000000)
  #ax.set_yscale('log')
  xlabel(xlab)
  ylabel(ylab)
  ...
  fig = figure()
  ax = fig.add_subplot(111)
  plt.hist(xFk, bins=100, align='left', log=True)
  xlim(-1,1)
  #ylim(1,1000000)
  #ax.set_yscale('log')
  xlabel(xlab)
  ...
  if pTJ != []:
    ...
    fig = figure()
    ax = fig.add_subplot(111)
    plt.hist(etaJ, bins=100, align='left')
    xlabel(xlab)
    ylabel(ylab)
    title(tle)
    plt.savefig(pdf,format='pdf')
    ...
    fig = figure()
    ax = fig.add_subplot(111)
    plt.hist(pTJ, bins=100, align='left', log=True)
    #ylim(1,1000000)
    #ax.set_yscale('log')
    xlabel(xlab)
    ylabel(ylab)
    ...
    fig = figure()
    ax = fig.add_subplot(111)
    plt.hist(xFJ, bins=100, align='left', log=True)
    xlim(-1,1)
    #ax.set_yscale('log')
    xlabel(xlab)
    ylabel(ylab)
