  ...
  for line in HEPfile:
    ...
    if iterator >4: #dumb workaround to get past the empty line and the first E line so 1st event is not zeros
      ...
      if parts[0] == 'P':
        ...
        if parts[2] == '91':
          numQGP+=1
        if parts[2] == '111' or  parts[2] =='211' or  parts[2] =='-211': #pion codes for pi0pi+pi-
          numpi+=1
        if parts[2] == '311' or  parts[2] =='321' or  parts[2] =='-321':
          numK+=1
...
# what to graph?  want numQGP but against what?, just histogram?
  ...
  pdf =PdfPages(filename.strip('.hepmc')+'.pdf')
  xlab="Number of non-unique QGP-like events"
  ylab="Counts"
  ...
  xlab="Number of non-unique QGP-like events"
  ylab="Number of all non-unique particles"
  ...
  xlab="Fraction of non-unique QGP-like events"
  ylab="Counts"
