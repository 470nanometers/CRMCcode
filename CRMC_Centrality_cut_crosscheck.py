...
### now to graphing #####
  ...
  for item in eventarray: #write output files here?
    ...
    if item.numP!=0 :
      ...
      if b > 4.5: #this is here for multiplicity cuts, lower b ==> higher multiplicity, b ~5 is ropughly 30-50% central? maybe?
...
#unfortunately this leads to empty bins in the method used here and therefore eliminates most of our signal (i.e. big blank graph at the end)
  ...
  c.Print(filename.strip('.hepmc')+'_ROOTCentralitycross.pdf[') #start pdf
  root.gROOT.Reset()
  ...
  c.Print(filename.strip('.hepmc')+'_ROOTCentralitycross.pdf') #fill multipage pdf
  ...
  c.Print(filename.strip('.hepmc')+'_ROOTCentralitycross.pdf]') #close pdf
