...
#unfortunately this leads to empty bins in the method used here and therefore eliminates most of our signal (i.e. big blank graph at the end)
  ...
  c.Print(filename.strip('.hepmc')+'_ROOTetaphiabsLEGO.pdf[') #start pdf
  for item in hists: #graph all events
    ...
    c.Print(filename.strip('.hepmc')+'_ROOTetaphiabsLEGO.pdf') #fill multipage pdf
    i+=1
  ...
  c.Print(filename.strip('.hepmc')+'_ROOTetaphiabsLEGO.pdf]') #close pdf
